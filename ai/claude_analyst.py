...
async def analyze_stock(
    ...
    tech_summary = (
        ...
        f"RSI(14): {tech.rsi:.1f if tech.rsi else 'N/A'}  "
        f"MACD: {tech.macd:.4f if tech.macd else 'N/A'}  Hist: {tech.macd_hist:+.4f if tech.macd_hist else 'N/A'}\n"
        f"Bollinger %B: {tech.bb_pct:.2f if tech.bb_pct else 'N/A'}  "
        f"ATR: {tech.atr:.2f if tech.atr else 'N/A'} ({tech.atr_pct:.1f if tech.atr_pct else 'N/A'}%)\n"
        f"Volume ratio (vs 20d avg): {tech.volume_ratio:.2f if tech.volume_ratio else 'N/A'}x\n"
        f"Stochastic K/D: {tech.stoch_k:.1f if tech.stoch_k else 'N/A'}/{tech.stoch_d:.1f if tech.stoch_d else 'N/A'}\n"
        f"52-week range: ${tech.week52_low:.2f if tech.week52_low else 'N/A'} – ${tech.week52_high:.2f if tech.week52_high else 'N/A'} "
        f"  ({tech.pct_from_52w_high:+.1f if tech.pct_from_52w_high else 'N/A'}% from 52w high)"
    )
    ...
    fund_summary = (
        ...
        f"Revenue: {_fmt_large(fund.revenue)} (YoY growth: {fund.revenue_growth_yoy:+.1f if fund.revenue_growth_yoy else 'N/A'}%)\n"
        f"Net Income: {_fmt_large(fund.net_income)}  |  Net Margin: {fund.net_margin:.1f if fund.net_margin else 'N/A'}%\n"
        f"EBITDA: {_fmt_large(fund.ebitda)}  |  FCF: {_fmt_large(fund.free_cash_flow)}\n"
        f"Gross Margin: {fund.gross_margin:.1f if fund.gross_margin else 'N/A'}%  "
        f"Operating Margin: {fund.operating_margin:.1f if fund.operating_margin else 'N/A'}%\n"
        f"P/E: {fund.pe_ratio:.1f if fund.pe_ratio else 'N/A'}x  "
        f"P/S: {fund.ps_ratio:.1f if fund.ps_ratio else 'N/A'}x  "
        f"P/B: {fund.pb_ratio:.1f if fund.pb_ratio else 'N/A'}x  "
        f"EV/EBITDA: {fund.ev_ebitda:.1f if fund.ev_ebitda else 'N/A'}x\n"
        f"Debt/Equity: {fund.debt_to_equity:.2f if fund.debt_to_equity else 'N/A'}  "
        f"Current Ratio: {fund.current_ratio:.2f if fund.current_ratio else 'N/A'}  "
        f"ROE: {fund.roe:.1f if fund.roe else 'N/A'}%  ROA: {fund.roa:.1f if fund.roa else 'N/A'}%\n"
